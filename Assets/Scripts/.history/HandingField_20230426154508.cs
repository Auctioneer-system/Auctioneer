...
public class HandingField : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		gcm = GetComponent<GenericCardManager>();
		// プレイヤーの登録処理
	...
	private void Update()
	{
		if(HandedScores.Count == countplayer)EndTurn();
		if (turnlimit-- == 0)
		{
	...
	public void EndTurn()
	{
		...
		{
			gcm.instance.SlideOutToLeft(HandedCards[0], 0.5f);
	...
	private GenericCardManager gcm;
	public void SubmitOthersScore(int cardnumber, string uid)
	{
		GameObject cardObject = gcm.instance.SpawnCardFromLeft(0.4f, 0.1f, cardnumber, 1);
		GenericCard submittedCard = cardObject.GetComponent<GenericCard>();
		submittedCard.isHanded = true;
		submittedCard.isDraggable = false;
	...
	private void GenerateLocalEnemyCards(){
		int rand =UnityEngine.Random.Range(0,Tekihuda.Count);
		SubmitOthersScore(Tekihuda[rand], "enemy");
		Tekihuda.RemoveAt(rand);
	}
